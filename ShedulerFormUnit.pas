...
begin
   ...
   begin
      ...
      begin
         ColWidths[I] := sgDaysOfWeek.ColWidths[1];
      end;

      for I := 0 to RowCount - 1 do
      begin
         if (I mod 5 = 0) then
            TempHeight := sgWeeksOfMonth.RowHeights[0];
         RowHeights[I] := TempHeight div (5 - I mod 5) - 4 ; //Возможно сделать лучше но и так пиздато
...
begin
   ...
   with Rect do begin   // нужно, чтобы избавиться от выделения
      Left := -1;
      Top := -1;
      Right := -1;
      Bottom := -1;
   ...
   begin
      ...
      Width := Self.ClientWidth - sgWeeksOfMonth.Width - 60;
   end;
   ChangeParametrs;
end;
...
var
   ...
   I, J, K: Integer;
begin
   Scheduler := CurrentSheduler.GetMonthScheduler();
   for I := 0 to High(Scheduler) do
   begin
      for J := 0 to High(Scheduler[0]) do
      begin
...
var
   Choice: Integer; // тип кнопки из диалогового окна
begin
